...
import { AiLogInput } from '@/components/AiLogInput/AiLogInput'
...
interface AddEntryPanelProps {
  ...
  projects: Project[]
  onAdd: (target: SearchTarget, activityType?: ActivityType, hours?: number, minutes?: number) => void
}
...
export function AddEntryPanel({ objectives, projects, onAdd }: AddEntryPanelProps) {
  ...
  const handleAiAdd = useCallback(
    (target: SearchTarget, hours: number, minutes: number) => {
      onAdd(target, undefined, hours, minutes)
    },
    [onAdd]
  )
    ...
    <div className={styles.panel}>
      ...
      <div className={styles.modeRow}>
        <button
          ...
          onClick={toggleAi}
          aria-pressed={aiMode}
          title={aiMode ? 'Switch to search' : 'Switch to AI mode'}
        >
